...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	$input_file_type = $output_file ['type'];
	$input_file_size = $output_file ['size'];
	...
	if($file_name!="" && ($file_type="image/jpeg"||$file_type="image/png"||$file_type="image/gif") &&  ($input_file_type="plain/text" || $input_file_type="plain/html")&& $file_size<=60000 && $output_file_size<=60000)
	{
		//Insert data 
	 // Check if assignment already exists
		...
		else
		{
			...
			$sql = "insert into assignment_submission(enrollment_id,sem_id,class_id,course_id,batch_id,input_file,output_file,submission_date,attempted) values('$enrollment_id','$sem_id','$class_id','$course_id','$batch_id','$input_file','$output_file','$date',1)";
