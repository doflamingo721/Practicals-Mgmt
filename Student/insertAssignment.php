...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if($file_name!="" && ($file_type="image/jpeg"||$file_type="image/png"||$file_type="image/gif") &&  ($input_file_type="plain/text" || $input_file_type="plain/html")&& $file_size<=60000 && $input_file_size<=60000)
	{
		//Insert data
	 // Check if assignment already exists
		...
		if($result->num_rows > 0)
		{
			...
			header("refresh:0; url=index.php#assignmentUpload");
		}
		else
		{
			// Insert student into database
			$sql = "insert into assignment_submission(assignment_no,enrollment_id,sem_id,class_id,course_id,batch_id,input_file,output_image,submission_date,attempted) values('$assignment_no','$enrollment_id','$sem_id','$class_id','$course_id','$batch_id','$input_file','$output_file','$date',0)";
			...
			{
				echo $mysqli->error;
				echo "<script>alert('Assignment could not be uploaded')</script>" ;
				header("refresh:0; url=index.php#assignmentUpload");
			}
		}
	}
	...
	{
		echo "<script>alert('Invalid input/output file. Check Rules for uploading')</script>" ;
		header("refresh:0; url=index.php#assignmentUpload");
	}
